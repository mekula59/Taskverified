...
export function PosterHomePage() {
  const auth = useAuth();
  const metrics = getPosterDashboardMetrics(auth.user?.id ?? "");
              ...
              <div key={task.id} className="rounded-2xl border border-border/60 bg-background/70 p-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-muted-foreground">{task.claimCount} of {task.claimLimit} claims used</div>
                  </div>
